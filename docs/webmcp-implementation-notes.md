...
- A live Codex browser smoke test at `https://clear.seufert.sh/board` discovered the workspace tool surface and successfully invoked `get_console_overview`. The public home route correctly exposed no tools because workspace state is not active there.
- Clear validates the documented tool-name and description budgets, including top-level descriptions for every input parameter.
