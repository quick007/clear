...
- Product-state SSE events use a PostgreSQL transactional outbox and `LISTEN`/`NOTIFY`; telemetry refresh hints use bounded Effect PubSub and are recoverable by re-querying current state. Do not introduce Redis for this path.
